...
#define _POSIX_C_SOURCE 200809L  // For dup2
...
bool do_system(const char* cmd) {
  ...
  int result = -1;
  result = system(cmd);  // NOLINT(concurrency-mt-unsafe, cert-env33-c)
  if (result != 0) {
    return false;
  }
...
bool do_exec_redirect(const char* outputfile, int count, ...) {
  ...
  // Copied and modified the code referenced from the link above
  const int fd = open(outputfile, O_WRONLY | O_CREAT | O_TRUNC, 0644);
  if (fd < 0) {
    perror("open");
    return false;
  }
